...
namespace math {
  ...
  LIBC_INLINE bfloat16 tgammabf16(bfloat16 x) {
    ...
    // The Lanczos evaluation uses binary64 intermediates to retain sufficient precision before rounding the result to bfloat16. The
    // hexadecimal literals were generated with Sollya:
    // > display = hexadecimal;
    // > round(pi, D, RN);
  ...
  // gamma is the Euler-Mascheroni constant. Its correctly rounded binary64
